...
const Docs = () => {
    ...
    Mondays - 4pm CET / 10am EST / 7am PST
    ...
    const initialize = async () => {
        toast.loading('Initializing Vector...')
        console.log('initialize')
        const res = await axios.post('/api/initialize')
        console.log('res: ', res.data)

        if (res.data.status === 'DOC_STORED') {
            toast.remove()
            toast.success('Vector Initialized!')
        } else {
            toast.remove()
    ...
    return (
        <>
            <main className='flex flex-col items-center justify-center py-8 space-y-4 bg-slate-900 h-screen relative font-exo'>
                ...
                <div className="h-5/6 w-5/6 text-white px-4 py-4 bg-slate-800 rounded-2xl relative  overflow-y-scroll scrollbar-thin scrollbar-track-slate-800 scrollbar-thumb-slate-200 scrollbar-corner-transparent">
                    <p className='whitespace-pre-line'>
                        {doc}
                    </p>
                </div>

                <div className='flex justify-center items-center space-x-2 absolute bottom-4'>
                    <button className="bg-blue-500 px-4 py-2 rounded-lg text-white" onClick={() => router.push('/')}>
                        Go Home
                    </button>
                    <button className="bg-blue-500 px-4 py-2 rounded-lg text-white" onClick={initialize}>
                        Initialize Vector
                    </button>
                </div>
            </main>
